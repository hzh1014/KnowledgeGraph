...
| 模块 | 路径 | 功能 |
|------|------|------|
| 数据预处理 | `backend/modules/prepare/` | 文本清洗、分句、标准化 |
| UIE 抽取 | `backend/modules/Uie-finetune/` | 基于 PaddleNLP 的信息抽取 |
| SPN4RE | `backend/modules/SPN4RE/` | 关系三元组联合抽取模型 |
| 图谱构建 | `backend/modules/knowledge_graph_builder.py` | 知识图谱构建与迭代 |
| API 服务 | `backend/server/main_api.py` | Flask 后端，集成 NER、图谱、Wikipedia、API |
| 前端界面 | `frontend/` | Vue 3 + ECharts 交互界面 |
...
```bash
# 创建 conda 环境
conda create -n chatzh python=3.10 -y
conda activate chatzh
...
将构建好的知识图谱数据放到 `backend/server/data/data.json`：

```bash
# 如果使用项目自带的示例数据，跳过此步骤
# 如果自行构建图谱：
python backend/main.py --project my_project --gpu 0
cp backend/data/my_project/iter_vN.json backend/server/data/data.json
```
...
系统支持多版本关系 Schema，在 `backend/data/schema/` 目录下定义：

```python
# backend/data/schema/schema_v4.py
schema = {
    "实体类型": ["设备", "人员", "技术", "方法"],
    "关系类型": {
        "设备": ["组成部分", "功能", "位置"],
...
```bash
# 将原始文本放入 backend/data/raw_data/
cp your_data.txt backend/data/raw_data/raw_data.txt
```
...
# 从检查点恢复
python backend/main.py --project my_project --resume checkpoint_v3 --gpu 0
```
...
A: 当前使用 jieba + 图谱实体匹配方案。确保 `backend/server/data/data.json` 中的实体名称准确。

### Q: 前端无法连接后端？

A: 确认后端已启动（`curl http://localhost:8000/`），检查 `frontend/.env.local` 中的 `VITE_API_URL` 配置。

### Q: 权重文件怎么获取？

A: `backend/weights/` 目录已加入 `.gitignore`，不会上传到 GitHub。权重文件会在首次运行时自动下载到 `~/.paddlenlp/taskflow/` 目录。
