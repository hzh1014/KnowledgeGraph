...
class ModelTrainer:
    ...
    def __init__(
        ...
        self.split_data()
        self.params: list = self._generate_running_cmd()

    def _generate_running_cmd(self) -> list:
        """
        Generate SPN4RE training command as argument list.

        Returns:
            List of command arguments for subprocess.run.
        """
        return [
            "python", "SPN4RE/main.py",
            "--bert_directory", self.model_name_or_path,
            "--max_epoch", str(settings.MAX_EPOCH),
            "--max_span_length", str(settings.MAX_SPAN_LENGTH),
            "--num_generated_triples", str(settings.NUM_GENERATED_TRIPLES),
            "--max_grad_norm", "2.5",
            "--na_rel_coef", "0.25",
            "--train_file", self.train_file,
            "--valid_file", self.valid_file,
            "--test_file", self.test_file,
            "--visible_gpu", self.gpu,
            "--generated_data_directory", self.generated_data_directory,
            "--generated_param_directory", self.generated_data_directory,
        ]
    ...
    def train_and_test(self) -> None:
        ...
        """
        logger.info(f"{green('Running:')} $ {' '.join(self.params)}")
