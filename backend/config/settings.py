...
class Settings:
    ...
    _cached_secret_key: Optional[str] = field(default=None, repr=False)
    ...
    def USE_API_MODE(self) -> bool:
        """Whether to use external LLM API."""
        return _get_env_bool("USE_API_MODE", False)

    @property
    def API_BASE_URL(self) -> str:
        """OpenAI-compatible API base URL."""
        return _get_env("API_BASE_URL", "https://api.deepseek.com/v1")

    @property
    def API_KEY(self) -> str:
        """API key for external LLM service."""
        return _get_env("API_KEY", "")

    @property
    def API_MODEL_NAME(self) -> str:
        """Model name for the API."""
        return _get_env("API_MODEL_NAME", "deepseek-chat")

    @property
    def HISTORY_WINDOW(self) -> int:
        """Max conversation rounds to send to LLM (0 = unlimited)."""
        return _get_env_int("HISTORY_WINDOW", 10)
