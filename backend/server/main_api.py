...
# Add backend/ to path for config/modules import
...
# API Configuration
USE_API_MODE = settings.USE_API_MODE
...
DATA_FILE = os.path.join(_backend_dir, 'server', 'data', 'data.json')
graph_data = {}
...
class BoundedCache(OrderedDict):
    """LRU cache with max size eviction."""
...
_wiki_cache = BoundedCache(maxsize=256)    # {query: {"title": ..., "summary": ...}}
_graph_cache = BoundedCache(maxsize=256)   # {entity_name: {"graph": ..., "triples": [...]}}
...
def chat():
    data = request.json
    if data is None:
        return error_response("Request body must be valid JSON with 'prompt' field", 400)
    user_input = data.get('prompt', '').strip()
    if not user_input:
        return error_response("'prompt' field is required and must not be empty", 400)
    ...
    if wiki_searcher is not None:
        ...
        if cache_key is not None and wiki["summary"] == "暂无相关描述":
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(wiki_search_task)
                try:
                    wiki_result = future.result(timeout=5)
            ...
            if wiki_result is not None:
                try:
                    wiki = {
                        "title": cc.convert(wiki_result.title),
                        "summary": cc.convert(wiki_result.summary[:500])
                    }
